...
our $SHORTDESCRIPTION = 'ExitPlugin redirects links to external sites via a page of your choice.';
...
#  set $redirectVia, $preMark, $postMark, $marksInLink
    ...
    while ( $initStage < $_[0] ) {
        ...
        if ( $initStage == 0 ) {
            ...
            Foswiki::Func::writeDebug( __PACKAGE__, "schemepat = ${schemepat}" ) if $debug;
            ...
            Foswiki::Func::writeDebug( __PACKAGE__, "noExit = ${noExit}" ) if $debug;
            ...
            $preMark = Foswiki::Func::expandCommonVariables( $preMark, $topic, $web );
            Foswiki::Func::writeDebug( __PACKAGE__, "preMark = ${preMark}" ) if $debug;
            $postMark = $Foswiki::cfg{Plugins}{$pluginName}{Postmark} || "";
            $postMark = Foswiki::Func::expandCommonVariables( $postMark, $topic, $web );
            Foswiki::Func::writeDebug( __PACKAGE__,  "postMark = ${postMark}" ) if $debug;
...
sub initPlugin
{
    ...
    Foswiki::Func::writeDebug( __PACKAGE__, "::initPlugin( $web.$topic ) is OK" ) if $debug;
    return 1;
}
...
### my ( $text ) = @_;   # do not uncomment, use $_[0] instead
    if ( $disable ) {
        Foswiki::Func::writeDebug( __PACKAGE__, "::endRenderingHandler(disabled by DISABLEEXITPLUGIN)" ) if $debug;
        return;
    }
    Foswiki::Func::writeDebug( __PACKAGE__, "::endRenderingHandler( $web.$topic )" ) if $debug;
    ...
    $_[0] =~ s/(<a)(\s+[^>]*?href=")($schemepat:\/\/[^"]+)("[^>]*>)(.*?)(<\/a>)/&_linkReplace($1,$2,$3,$4,$5,$6)/isgeo;
}
